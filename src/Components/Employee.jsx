...
export class Employee extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("http://192.168.100.18:8000/api/emplogin/", config)
      .then((response) => {
    ...
    axios
      .get("http://192.168.100.18:8000/api/employe/", config)
      .then((response) => {
        console.log(response.data);
        this.setState({
  ...
  onsubmit = (e) => {
    ...
    if (this.state.filter === "") {
      const empdata = this.state.employeedata.filter(
        (e) =>
          new Date(this.state.startdate) <= new Date(e.signin_time) &&
          new Date(this.state.enddate) >= new Date(e.signin_time)
      );
      console.log(empdata);
      ...
      const empData = this.state.employeedata.filter(
        (emp) => emp.employe_id == this.state.filter
      );
      const Data = empData.filter(
        (e) =>
          new Date(this.state.startdate) <= new Date(e.signin_time) &&
          new Date(this.state.enddate) >= new Date(e.signin_time)
      );
  ...
  render() {
    return (
      <div>
        <div className="row">
          <div className="col-sm-12 btn btn-info">employee login details</div>
        </div>
            ...
            <div className="col-sm-3 form-group">
              ...
              >
                ...
                {this.state.employee.map((emp) => (
                  <option value={emp.emp_id}>{emp.user_name}</option>
                ))}
              </select>
            </div>
            <div className="col-sm-3 form-group">
